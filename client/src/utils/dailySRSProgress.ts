import type { DailySRSProgress } from '@/types';

const DAILY_KEY = 'kanji_srs_daily_progress';

const todayString = (): string => new Date().toISOString().split('T')[0];

export const loadDailyProgress = (): DailySRSProgress | null => {
  const raw = localStorage.getItem(DAILY_KEY);
  if (!raw) return null;

  const data = JSON.parse(raw) as DailySRSProgress;
...
export const saveDailyProgress = (progress: DailySRSProgress) => {
