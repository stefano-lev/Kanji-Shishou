...
interface DeckFilterOptions {
  levels: JLPTLevel[];
  maxAccuracy?: number | null;
}

interface BuildDeckOptions extends DeckFilterOptions {
  randomOrder?: boolean;
  maxCards?: number | null;
}

export const getKanjiByLevels = (levels: JLPTLevel[]): Kanji[] => {
...
export const getKanjiCountByLevels = (levels: JLPTLevel[]): number => {
  return getKanjiByLevels(levels).length;
};

export const shuffleDeck = (deck: Kanji[]): Kanji[] => {
  return [...deck].sort(() => Math.random() - 0.5);
};

export const getFilteredDeckCount = ({
  levels,
  maxAccuracy = null,
}: DeckFilterOptions): number => {
